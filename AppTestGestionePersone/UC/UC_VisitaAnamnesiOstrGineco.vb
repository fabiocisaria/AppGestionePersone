...
Public Class UC_VisitaAnamnesiOstrGineco
    ...
    Private Async Sub FormVisitaAnamnesiOstrGineco_Shown(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        With ComboBoxMenopausa
            ' Imposta i dati
            .DataSource = New List(Of String) From {"No", "< 1 anno", "1 - 3 anni", "> 3 anni"}
            .SelectedIndex = -1
        End With

        With ComboBoxPartiOperativiTipo
            ' Imposta i dati
            .DataSource = New List(Of String) From {"Ventosa ostetrica", "Kristeller", "Ventosa ostetrica + Kristeller"}
            .SelectedIndex = -1
        End With
    ...
    Private Async Sub CaricaContraccAsync()
        ComboBoxTipoContracc.Enabled = False
        Try
            Dim query As String = "SELECT ID, Tipo FROM TipiContraccettivo ORDER BY Tipo"
            Dim dt As DataTable = Await ConnessioneDB.EseguiQueryAsync(query)
            ComboBoxTipoContracc.DataSource = dt
            ComboBoxTipoContracc.DisplayMember = "Tipo"
            ComboBoxTipoContracc.ValueMember = "ID"
        Catch ex As Exception
            MessageBox.Show("Errore nel caricamento della lista parentela: " & ex.Message)
        Finally
            ComboBoxTipoContracc.Enabled = True
        End Try
    End Sub
    ...
    Private Function CheckSelezione() As Boolean
        ...
        If RadioButtonAmenorreaSi.Checked Then
            If ComboBoxMenopausa.SelectedIndex = -1 OrElse Not Utils.IsValidSelection(ComboBoxMenopausa) Then
        ...
        If RadioButtonUsoContraccSi.Checked Then
            If ComboBoxTipoContracc.SelectedIndex = -1 OrElse Not Utils.IsValidSelection(ComboBoxTipoContracc) Then
        ...
        If RadioButtonPartiOperativiSi.Checked Then
            If ComboBoxPartiOperativiTipo.SelectedIndex = -1 OrElse Not Utils.IsValidSelection(ComboBoxPartiOperativiTipo) Then
    ...
    Private Async Function CercaVisitaAsync() As Task(Of Boolean)
        ...
        If dtCheckGineco.Rows.Count = 1 And dtCheckOstetrico.Rows.Count = 1 Then
            ...
            If RadioButtonUsoContraccSi.Checked Then
                ' Tipo contraccettivo
                ComboBoxTipoContracc.SelectedValue = CInt(dettagliVisitaGineco("ID_Contraccettivo"))
            Else
            ...
            If IsDBNull(dettagliVisitaGineco("Menopausa").ToString().Trim()) Then
                ComboBoxMenopausa.SelectedIndex = -1
            Else
                If dettagliVisitaGineco("Menopausa").ToString().Trim() = "No" Then
                    ComboBoxMenopausa.SelectedIndex = 0
            ...
            Dim valoreDB As String = dettagliVisitaGineco("Menopausa").ToString().Trim()

            ComboBoxMenopausa.Text = dettagliVisitaGineco("Menopausa").ToString().Trim()
